...
from ema_workbench import (Model, RealParameter,CategoricalParameter, IntegerParameter, Policy, TimeSeriesOutcome, ema_logging, perform_experiments)
...
# outdir = "C:\\Users\\wb558960\\OneDrive - WBG\\CCDRs LAC\\Argentina\\DeepDives\\Vaca Muerta\\Python\\outputs_rdm\\outputs_v6\\10pol_300\\"
...
outdir = f"outputs_rdm\\outputs_v7\\32pol_{num_experiments}/"
...
def VacaMuerta(yr0 = 2020, 
    ...
    def calculate_state(t,w,k):
        ...
        if (k == 'Gas'):
            ...
            if(exports[exports['year']==t]['gas_ex_cap'].squeeze()>=gas_ex_cap_end):
               ds_capex = 0 
        else:  ds_capex = 0
    ...
    for t in range(yr0,final_yr+1):
        ...
        #CONSUMPTION
            ...
            # res_cons = (consumption('natural gas','residential','gas_cons_residential','Cons Gas Local ktoe',a_gas_res) +
            #             consumption('oil products','residential','oil_cons_residential','Cons Oil Local ktoe',a_oil_res) + 
            #             consumption('electricity','residential','el_cons_residential','Cons El Local ktoe',a_el_res)
            # )

            # ind_cons = (consumption('natural gas','industry','gas_cons_industry','Cons Gas Local ktoe', a_gas_ind)+
            #             consumption('oil products','industry','oil_cons_industry','Cons Oil Local ktoe',a_oil_ind)+
            #             consumption('electricity','industry','el_cons_industry','Cons El Local ktoe',a_el_ind)
            # )

            # trans_cons = (consumption('natural gas','general','gas_cons_transport','Cons Gas Local ktoe', a_gas_ind)+
            #             consumption('oil products','general','oil_cons_transport','Cons Oil Local ktoe',a_oil_trans)+
            #             consumption('electricity','general','el_cons_transport','Cons El Local ktoe',a_el_trans)
            # )

            # other_cons = (consumption('natural gas','general','gas_cons_other','Cons Gas Local ktoe',0)+
            #             consumption('oil products','general','oil_cons_other','Cons Oil Local ktoe',0) +
            #             consumption('electricity','general','el_cons_other','Cons Oil Local ktoe',0)
            # )
    ...
    # discount factor calculation - in RDM
    # fiscal=fiscal.reset_index()
    # fiscal['discount_factor'] = 1/(1*(1 + wacc)**(fiscal['year']-yr0-1))
    fiscal['col_name'] = fiscal['product']+ "_" + fiscal['well_type']
    fiscal_td = fiscal.pivot(index="year", columns=["col_name"],values=["price","ds_capex",'opex','domestic revenue','export revenue','production subsidy','import subsidy','royalties','export duties','trade balance','us_capex','starts','discount_factor'])  
    ...
    fiscal['total_private_capex'] = fiscal['us_capex'] + fiscal['ds_capex']*public_ds_capex_share
    ...
    fiscal2 = fiscal.reset_index().groupby(['year', 'product']).agg({
        ...
        'total_private_capex':sum,    # Sum duration per group
    ...
    # print(fiscal2.head())
    macro_inputs = fiscal2.pivot(index="year", columns=["product"],values=["price","intl_pr","ds_capex",'opex','domestic revenue','export revenue','production subsidy','import subsidy','royalties','export duties','trade balance','us_capex','starts','discount_factor', 'total_capex','total_private_capex'])  
    ...
    macro_inputs['foreign_capex Oil'] = macro_inputs['total_private_capex Oil']*foreign_capex_share
    macro_inputs['foreign_capex Gas'] = macro_inputs['total_private_capex Gas']*foreign_capex_share
    ...
    finance['trade_balance'] = fiscal['discount_factor']*fiscal['trade balance']
    ...
    total_tb_gdp = finance['trade_balance'].agg('sum')/GDP*100
    ...
    return npv_gdp, npv_unsub_gdp,total_tb_gdp, total_ft_gdp, wells_total, gdp_npv_conv_gas,gdp_npv_conv_oil,gdp_npv_unconv_gas,gdp_npv_unconv_oil, ft_npv_conv_gas, ft_npv_conv_oil, ft_npv_unconv_gas, ft_npv_unconv_oil, fiscal_td, exports
...
#22 uncertainties
    model.uncertainties = [CategoricalParameter('rcp',['CURR','1.5']), #emissions pathway
                            CategoricalParameter('model',['POLES','WB','GCAM','MESSAGE','REMIND']),
                            #dropped remind: message has better energy,'REMIND', dropped gcam - POLES has more reasonable representation of Arg energy 'GCAM',
                            RealParameter("discount",.02, .11), #price elasticity of supply - oil 
                            ...
                            # RealParameter("t_rate_gas",-.05, 0.05), #transition rate towards gas 
                            # RealParameter("t_rate_oil",-.05, 0.05), #transition rate towards oil
                            RealParameter("t_rate_e",-.05, .05), #transition rate towards electricity
                            
                            RealParameter("us_capex_gas",2000000, 15000000), #Cost of upstream capex expenditure 
                            RealParameter("us_capex_oil",2000000, 15000000), #Cost of upstream capex expenditure 
                            RealParameter("m",2000, 35000), #cost of downstream gas capital expenditure
                            ...
                            RealParameter("unconv_prod_decline_end_gas", 0,.15), #exogenous production decline in response to decarbonization
                            RealParameter("unconv_prod_decline_end_oil", 0,.15), #exogenous production decline in response to decarbonization
                            # RealParameter("foreign_capex_share", 0,.40), #exogenous production decline in response to decarbonization
                            IntegerParameter("well_life",5 ,35), #expected lifetime of an active well
                            ...
                            IntegerParameter("gas_demand_decline_speed",3 ,30),#years over which international demand for oil declines
                            IntegerParameter("oil_demand_decline_speed",3 ,30)#years over which international demand for oil declines
                            ]      

    model.outcomes = [ScalarOutcome("npv_gdp"),
                       ScalarOutcome("npv_unsub_gdp"),
                       ScalarOutcome("total_tb_gdp"),
                       ScalarOutcome("total_ft_gdp"),
                       ScalarOutcome("wells_total"), 
                       ScalarOutcome("gdp_npv_conv_gas"),
                       ScalarOutcome("gdp_npv_conv_oil"),
                       ScalarOutcome("gdp_npv_unconv_gas"),  
                       ScalarOutcome("gdp_npv_unconv_oil"), 
                       ScalarOutcome("gdp_ft_conv_gas"),
                       ScalarOutcome("gdp_ft_conv_oil"),
                       ScalarOutcome("gdp_ft_unconv_gas"),  
                       ScalarOutcome("gdp_ft_unconv_oil"),
                       ArrayOutcome("fiscal_td"),
                       ArrayOutcome("exports")
                    #    TimeSeriesOutcome('trade_out'),
                    #    TimeSeriesOutcome('prod_conv_out'),
                    #    TimeSeriesOutcome('prod_unconv_out'),
                    #    TimeSeriesOutcome('cons_out')
                      ] 
    model.levers =  [
                    RealParameter("public_ds_capex_share",0.0, 1.0), #share of downstream capital provided by public
                    RealParameter("foreign_capex_share",0.0, 1.0), #sensitivity of prices to to stock var
                    RealParameter("profit_tax_rate",0.0, .5),
                    ...
                    # RealParameter("price_threshold_gas",4.0,6.0), #price threshold for payment of export duties
                    ...
                    RealParameter('cons_wedge_end_gas',.25,8.0), #Wedge between domestic producer prices and domestic producer prices
                    RealParameter("cons_wedge_end_oil",.25,8.0), #Wedge between domestic producer prices and domestic producer prices
                    RealParameter('intl_wedge_end_gas',.25,8.0), #Wedge between international producer prices and domestic producer prices
                    RealParameter("intl_wedge_end_oil",.25,8.0), #Wedge between international producer prices and domestic producer prices
                    RealParameter("a_gas_trans",0.0, .06),  #energy efficiency improvement rate transport gas 
                    RealParameter("a_gas_res",0.0, .06),  #energy efficiency improvement rate residential gas 
                    RealParameter("a_gas_ind",0.0, .06),  #energy efficiency improvement rate industrial gas  
                    RealParameter("a_oil_trans",0.0, .06),  #energy efficiency improvement rate transport oil 
                    RealParameter("a_oil_res",0.0, .06),  #energy efficiency improvement rate residential oil  
                    RealParameter("a_oil_ind",0.0, .06),  #energy efficiency improvement rate industrial oil  
                    RealParameter("a_el_trans",0.0, .06),  #energy efficiency improvement rate residential electricity 
                    RealParameter("a_el_res",0.0, .06), #energy efficiency improvement rate residential electricity 
                    RealParameter("a_el_ind",0.0, .06), #energy efficiency improvement rate industrial electricity
                    RealParameter("gas_ex_cap_end",1905 , 110000), #final level of export demand internationally
    ...
    policies = [Policy('1',public_ds_capex_share=0.67,share_covered=0.17,prod_subsidy_p_ratio=1.3,gas_ex_cap_end=110000,gas_ex_cap_increase=30,oil_ex_cap_end=76650,oil_ex_cap_increase=30,royalty_rate_gas=0.12,duty_rate_gas=0.08,royalty_rate_oil=0.12,duty_rate_oil=0.08,profit_tax_rate=0.3,cons_wedge_end_gas=3.5,cons_wedge_end_oil=1.5,intl_wedge_end_gas=2,intl_wedge_end_oil=1.5,a_gas_trans=0.03,a_gas_res=0.03,a_gas_ind=0.03,a_oil_trans=0.03,a_oil_res=0.03,a_oil_ind=0.03,a_el_trans=0.03,a_el_res=0.03,a_el_ind=0.03),
                Policy('2',public_ds_capex_share=0.67,share_covered=0.17,prod_subsidy_p_ratio=1.3,gas_ex_cap_end=110000,gas_ex_cap_increase=30,oil_ex_cap_end=76650,oil_ex_cap_increase=30,royalty_rate_gas=0.05,duty_rate_gas=0,royalty_rate_oil=0.05,duty_rate_oil=0,profit_tax_rate=0.15,cons_wedge_end_gas=3.5,cons_wedge_end_oil=1.5,intl_wedge_end_gas=2,intl_wedge_end_oil=1.5,a_gas_trans=0.03,a_gas_res=0.03,a_gas_ind=0.03,a_oil_trans=0.03,a_oil_res=0.03,a_oil_ind=0.03,a_el_trans=0.03,a_el_res=0.03,a_el_ind=0.03),
                Policy('3',public_ds_capex_share=0,share_covered=0,prod_subsidy_p_ratio=1,gas_ex_cap_end=110000,gas_ex_cap_increase=30,oil_ex_cap_end=76650,oil_ex_cap_increase=30,royalty_rate_gas=0.12,duty_rate_gas=0.08,royalty_rate_oil=0.12,duty_rate_oil=0.08,profit_tax_rate=0.3,cons_wedge_end_gas=3.5,cons_wedge_end_oil=1.5,intl_wedge_end_gas=2,intl_wedge_end_oil=1.5,a_gas_trans=0.03,a_gas_res=0.03,a_gas_ind=0.03,a_oil_trans=0.03,a_oil_res=0.03,a_oil_ind=0.03,a_el_trans=0.03,a_el_res=0.03,a_el_ind=0.03),
                Policy('4',public_ds_capex_share=0,share_covered=0,prod_subsidy_p_ratio=1,gas_ex_cap_end=110000,gas_ex_cap_increase=30,oil_ex_cap_end=76650,oil_ex_cap_increase=30,royalty_rate_gas=0.05,duty_rate_gas=0,royalty_rate_oil=0.05,duty_rate_oil=0,profit_tax_rate=0.15,cons_wedge_end_gas=3.5,cons_wedge_end_oil=1.5,intl_wedge_end_gas=2,intl_wedge_end_oil=1.5,a_gas_trans=0.03,a_gas_res=0.03,a_gas_ind=0.03,a_oil_trans=0.03,a_oil_res=0.03,a_oil_ind=0.03,a_el_trans=0.03,a_el_res=0.03,a_el_ind=0.03),
                Policy('5',public_ds_capex_share=0.67,share_covered=0.17,prod_subsidy_p_ratio=1.3,gas_ex_cap_end=110000,gas_ex_cap_increase=30,oil_ex_cap_end=76650,oil_ex_cap_increase=30,royalty_rate_gas=0.12,duty_rate_gas=0.08,royalty_rate_oil=0.12,duty_rate_oil=0.08,profit_tax_rate=0.3,cons_wedge_end_gas=.8,cons_wedge_end_oil=.8,intl_wedge_end_gas=1,intl_wedge_end_oil=1,a_gas_trans=0.03,a_gas_res=0.03,a_gas_ind=0.03,a_oil_trans=0.03,a_oil_res=0.03,a_oil_ind=0.03,a_el_trans=0.03,a_el_res=0.03,a_el_ind=0.03),
                Policy('6',public_ds_capex_share=0.67,share_covered=0.17,prod_subsidy_p_ratio=1.3,gas_ex_cap_end=110000,gas_ex_cap_increase=30,oil_ex_cap_end=76650,oil_ex_cap_increase=30,royalty_rate_gas=0.05,duty_rate_gas=0,royalty_rate_oil=0.05,duty_rate_oil=0,profit_tax_rate=0.15,cons_wedge_end_gas=.8,cons_wedge_end_oil=.8,intl_wedge_end_gas=1,intl_wedge_end_oil=1,a_gas_trans=0.03,a_gas_res=0.03,a_gas_ind=0.03,a_oil_trans=0.03,a_oil_res=0.03,a_oil_ind=0.03,a_el_trans=0.03,a_el_res=0.03,a_el_ind=0.03),
                Policy('7',public_ds_capex_share=0,share_covered=0,prod_subsidy_p_ratio=1,gas_ex_cap_end=110000,gas_ex_cap_increase=30,oil_ex_cap_end=76650,oil_ex_cap_increase=30,royalty_rate_gas=0.12,duty_rate_gas=0.08,royalty_rate_oil=0.12,duty_rate_oil=0.08,profit_tax_rate=0.3,cons_wedge_end_gas=.8,cons_wedge_end_oil=.8,intl_wedge_end_gas=1,intl_wedge_end_oil=1,a_gas_trans=0.03,a_gas_res=0.03,a_gas_ind=0.03,a_oil_trans=0.03,a_oil_res=0.03,a_oil_ind=0.03,a_el_trans=0.03,a_el_res=0.03,a_el_ind=0.03),
                Policy('8',public_ds_capex_share=0,share_covered=0,prod_subsidy_p_ratio=1,gas_ex_cap_end=110000,gas_ex_cap_increase=30,oil_ex_cap_end=76650,oil_ex_cap_increase=30,royalty_rate_gas=0.05,duty_rate_gas=0,royalty_rate_oil=0.05,duty_rate_oil=0,profit_tax_rate=0.15,cons_wedge_end_gas=.8,cons_wedge_end_oil=.8,intl_wedge_end_gas=1,intl_wedge_end_oil=1,a_gas_trans=0.03,a_gas_res=0.03,a_gas_ind=0.03,a_oil_trans=0.03,a_oil_res=0.03,a_oil_ind=0.03,a_el_trans=0.03,a_el_res=0.03,a_el_ind=0.03),
                Policy('9',public_ds_capex_share=0.67,share_covered=0.17,prod_subsidy_p_ratio=1.3,gas_ex_cap_end=110000,gas_ex_cap_increase=30,oil_ex_cap_end=76650,oil_ex_cap_increase=30,royalty_rate_gas=0.12,duty_rate_gas=0.08,royalty_rate_oil=0.12,duty_rate_oil=0.08,profit_tax_rate=0.3,cons_wedge_end_gas=3.5,cons_wedge_end_oil=1.5,intl_wedge_end_gas=2,intl_wedge_end_oil=1.5,a_gas_trans=0.005,a_gas_res=0.005,a_gas_ind=0.005,a_oil_trans=0.005,a_oil_res=0.005,a_oil_ind=0.005,a_el_trans=0.005,a_el_res=0.005,a_el_ind=0.005),
                Policy('10',public_ds_capex_share=0.67,share_covered=0.17,prod_subsidy_p_ratio=1.3,gas_ex_cap_end=110000,gas_ex_cap_increase=30,oil_ex_cap_end=76650,oil_ex_cap_increase=30,royalty_rate_gas=0.05,duty_rate_gas=0,royalty_rate_oil=0.05,duty_rate_oil=0,profit_tax_rate=0.15,cons_wedge_end_gas=3.5,cons_wedge_end_oil=1.5,intl_wedge_end_gas=2,intl_wedge_end_oil=1.5,a_gas_trans=0.005,a_gas_res=0.005,a_gas_ind=0.005,a_oil_trans=0.005,a_oil_res=0.005,a_oil_ind=0.005,a_el_trans=0.005,a_el_res=0.005,a_el_ind=0.005),
                Policy('11',public_ds_capex_share=0,share_covered=0,prod_subsidy_p_ratio=1,gas_ex_cap_end=110000,gas_ex_cap_increase=30,oil_ex_cap_end=76650,oil_ex_cap_increase=30,royalty_rate_gas=0.12,duty_rate_gas=0.08,royalty_rate_oil=0.12,duty_rate_oil=0.08,profit_tax_rate=0.3,cons_wedge_end_gas=3.5,cons_wedge_end_oil=1.5,intl_wedge_end_gas=2,intl_wedge_end_oil=1.5,a_gas_trans=0.005,a_gas_res=0.005,a_gas_ind=0.005,a_oil_trans=0.005,a_oil_res=0.005,a_oil_ind=0.005,a_el_trans=0.005,a_el_res=0.005,a_el_ind=0.005),
                Policy('12',public_ds_capex_share=0,share_covered=0,prod_subsidy_p_ratio=1,gas_ex_cap_end=110000,gas_ex_cap_increase=30,oil_ex_cap_end=76650,oil_ex_cap_increase=30,royalty_rate_gas=0.05,duty_rate_gas=0,royalty_rate_oil=0.05,duty_rate_oil=0,profit_tax_rate=0.15,cons_wedge_end_gas=3.5,cons_wedge_end_oil=1.5,intl_wedge_end_gas=2,intl_wedge_end_oil=1.5,a_gas_trans=0.005,a_gas_res=0.005,a_gas_ind=0.005,a_oil_trans=0.005,a_oil_res=0.005,a_oil_ind=0.005,a_el_trans=0.005,a_el_res=0.005,a_el_ind=0.005),
                Policy('13',public_ds_capex_share=0.67,share_covered=0.17,prod_subsidy_p_ratio=1.3,gas_ex_cap_end=110000,gas_ex_cap_increase=30,oil_ex_cap_end=76650,oil_ex_cap_increase=30,royalty_rate_gas=0.12,duty_rate_gas=0.08,royalty_rate_oil=0.12,duty_rate_oil=0.08,profit_tax_rate=0.3,cons_wedge_end_gas=.8,cons_wedge_end_oil=.8,intl_wedge_end_gas=1,intl_wedge_end_oil=1,a_gas_trans=0.005,a_gas_res=0.005,a_gas_ind=0.005,a_oil_trans=0.005,a_oil_res=0.005,a_oil_ind=0.005,a_el_trans=0.005,a_el_res=0.005,a_el_ind=0.005),
                Policy('14',public_ds_capex_share=0.67,share_covered=0.17,prod_subsidy_p_ratio=1.3,gas_ex_cap_end=110000,gas_ex_cap_increase=30,oil_ex_cap_end=76650,oil_ex_cap_increase=30,royalty_rate_gas=0.05,duty_rate_gas=0,royalty_rate_oil=0.05,duty_rate_oil=0,profit_tax_rate=0.15,cons_wedge_end_gas=.8,cons_wedge_end_oil=.8,intl_wedge_end_gas=1,intl_wedge_end_oil=1,a_gas_trans=0.005,a_gas_res=0.005,a_gas_ind=0.005,a_oil_trans=0.005,a_oil_res=0.005,a_oil_ind=0.005,a_el_trans=0.005,a_el_res=0.005,a_el_ind=0.005),
                Policy('15',public_ds_capex_share=0,share_covered=0,prod_subsidy_p_ratio=1,gas_ex_cap_end=110000,gas_ex_cap_increase=30,oil_ex_cap_end=76650,oil_ex_cap_increase=30,royalty_rate_gas=0.12,duty_rate_gas=0.08,royalty_rate_oil=0.12,duty_rate_oil=0.08,profit_tax_rate=0.3,cons_wedge_end_gas=.8,cons_wedge_end_oil=.8,intl_wedge_end_gas=1,intl_wedge_end_oil=1,a_gas_trans=0.005,a_gas_res=0.005,a_gas_ind=0.005,a_oil_trans=0.005,a_oil_res=0.005,a_oil_ind=0.005,a_el_trans=0.005,a_el_res=0.005,a_el_ind=0.005),
                Policy('16',public_ds_capex_share=0,share_covered=0,prod_subsidy_p_ratio=1,gas_ex_cap_end=110000,gas_ex_cap_increase=30,oil_ex_cap_end=76650,oil_ex_cap_increase=30,royalty_rate_gas=0.05,duty_rate_gas=0,royalty_rate_oil=0.05,duty_rate_oil=0,profit_tax_rate=0.15,cons_wedge_end_gas=.8,cons_wedge_end_oil=.8,intl_wedge_end_gas=1,intl_wedge_end_oil=1,a_gas_trans=0.005,a_gas_res=0.005,a_gas_ind=0.005,a_oil_trans=0.005,a_oil_res=0.005,a_oil_ind=0.005,a_el_trans=0.005,a_el_res=0.005,a_el_ind=0.005),
                Policy('17',public_ds_capex_share=0.67,share_covered=0.17,prod_subsidy_p_ratio=1.3,gas_ex_cap_end=4631,gas_ex_cap_increase=3,oil_ex_cap_end=76650,oil_ex_cap_increase=30,royalty_rate_gas=0.12,duty_rate_gas=0.08,royalty_rate_oil=0.12,duty_rate_oil=0.08,profit_tax_rate=0.3,cons_wedge_end_gas=3.5,cons_wedge_end_oil=1.5,intl_wedge_end_gas=2,intl_wedge_end_oil=1.5,a_gas_trans=0.03,a_gas_res=0.03,a_gas_ind=0.03,a_oil_trans=0.03,a_oil_res=0.03,a_oil_ind=0.03,a_el_trans=0.03,a_el_res=0.03,a_el_ind=0.03),
                Policy('18',public_ds_capex_share=0.67,share_covered=0.17,prod_subsidy_p_ratio=1.3,gas_ex_cap_end=4631,gas_ex_cap_increase=3,oil_ex_cap_end=76650,oil_ex_cap_increase=30,royalty_rate_gas=0.05,duty_rate_gas=0,royalty_rate_oil=0.05,duty_rate_oil=0,profit_tax_rate=0.15,cons_wedge_end_gas=3.5,cons_wedge_end_oil=1.5,intl_wedge_end_gas=2,intl_wedge_end_oil=1.5,a_gas_trans=0.03,a_gas_res=0.03,a_gas_ind=0.03,a_oil_trans=0.03,a_oil_res=0.03,a_oil_ind=0.03,a_el_trans=0.03,a_el_res=0.03,a_el_ind=0.03),
                Policy('19',public_ds_capex_share=0,share_covered=0,prod_subsidy_p_ratio=1,gas_ex_cap_end=4631,gas_ex_cap_increase=3,oil_ex_cap_end=76650,oil_ex_cap_increase=30,royalty_rate_gas=0.12,duty_rate_gas=0.08,royalty_rate_oil=0.12,duty_rate_oil=0.08,profit_tax_rate=0.3,cons_wedge_end_gas=3.5,cons_wedge_end_oil=1.5,intl_wedge_end_gas=2,intl_wedge_end_oil=1.5,a_gas_trans=0.03,a_gas_res=0.03,a_gas_ind=0.03,a_oil_trans=0.03,a_oil_res=0.03,a_oil_ind=0.03,a_el_trans=0.03,a_el_res=0.03,a_el_ind=0.03),
                Policy('20',public_ds_capex_share=0,share_covered=0,prod_subsidy_p_ratio=1,gas_ex_cap_end=4631,gas_ex_cap_increase=3,oil_ex_cap_end=76650,oil_ex_cap_increase=30,royalty_rate_gas=0.05,duty_rate_gas=0,royalty_rate_oil=0.05,duty_rate_oil=0,profit_tax_rate=0.15,cons_wedge_end_gas=3.5,cons_wedge_end_oil=1.5,intl_wedge_end_gas=2,intl_wedge_end_oil=1.5,a_gas_trans=0.03,a_gas_res=0.03,a_gas_ind=0.03,a_oil_trans=0.03,a_oil_res=0.03,a_oil_ind=0.03,a_el_trans=0.03,a_el_res=0.03,a_el_ind=0.03),
                Policy('21',public_ds_capex_share=0.67,share_covered=0.17,prod_subsidy_p_ratio=1.3,gas_ex_cap_end=4631,gas_ex_cap_increase=3,oil_ex_cap_end=76650,oil_ex_cap_increase=30,royalty_rate_gas=0.12,duty_rate_gas=0.08,royalty_rate_oil=0.12,duty_rate_oil=0.08,profit_tax_rate=0.3,cons_wedge_end_gas=.8,cons_wedge_end_oil=.8,intl_wedge_end_gas=1,intl_wedge_end_oil=1,a_gas_trans=0.03,a_gas_res=0.03,a_gas_ind=0.03,a_oil_trans=0.03,a_oil_res=0.03,a_oil_ind=0.03,a_el_trans=0.03,a_el_res=0.03,a_el_ind=0.03),
                Policy('22',public_ds_capex_share=0.67,share_covered=0.17,prod_subsidy_p_ratio=1.3,gas_ex_cap_end=4631,gas_ex_cap_increase=3,oil_ex_cap_end=76650,oil_ex_cap_increase=30,royalty_rate_gas=0.05,duty_rate_gas=0,royalty_rate_oil=0.05,duty_rate_oil=0,profit_tax_rate=0.15,cons_wedge_end_gas=.8,cons_wedge_end_oil=.8,intl_wedge_end_gas=1,intl_wedge_end_oil=1,a_gas_trans=0.03,a_gas_res=0.03,a_gas_ind=0.03,a_oil_trans=0.03,a_oil_res=0.03,a_oil_ind=0.03,a_el_trans=0.03,a_el_res=0.03,a_el_ind=0.03),
                Policy('23',public_ds_capex_share=0,share_covered=0,prod_subsidy_p_ratio=1,gas_ex_cap_end=4631,gas_ex_cap_increase=3,oil_ex_cap_end=76650,oil_ex_cap_increase=30,royalty_rate_gas=0.12,duty_rate_gas=0.08,royalty_rate_oil=0.12,duty_rate_oil=0.08,profit_tax_rate=0.3,cons_wedge_end_gas=.8,cons_wedge_end_oil=.8,intl_wedge_end_gas=1,intl_wedge_end_oil=1,a_gas_trans=0.03,a_gas_res=0.03,a_gas_ind=0.03,a_oil_trans=0.03,a_oil_res=0.03,a_oil_ind=0.03,a_el_trans=0.03,a_el_res=0.03,a_el_ind=0.03),
                Policy('24',public_ds_capex_share=0,share_covered=0,prod_subsidy_p_ratio=1,gas_ex_cap_end=4631,gas_ex_cap_increase=3,oil_ex_cap_end=76650,oil_ex_cap_increase=30,royalty_rate_gas=0.05,duty_rate_gas=0,royalty_rate_oil=0.05,duty_rate_oil=0,profit_tax_rate=0.15,cons_wedge_end_gas=.8,cons_wedge_end_oil=.8,intl_wedge_end_gas=1,intl_wedge_end_oil=1,a_gas_trans=0.03,a_gas_res=0.03,a_gas_ind=0.03,a_oil_trans=0.03,a_oil_res=0.03,a_oil_ind=0.03,a_el_trans=0.03,a_el_res=0.03,a_el_ind=0.03),
                Policy('25',public_ds_capex_share=0.67,share_covered=0.17,prod_subsidy_p_ratio=1.3,gas_ex_cap_end=4631,gas_ex_cap_increase=3,oil_ex_cap_end=76650,oil_ex_cap_increase=30,royalty_rate_gas=0.12,duty_rate_gas=0.08,royalty_rate_oil=0.12,duty_rate_oil=0.08,profit_tax_rate=0.3,cons_wedge_end_gas=3.5,cons_wedge_end_oil=1.5,intl_wedge_end_gas=2,intl_wedge_end_oil=1.5,a_gas_trans=0.005,a_gas_res=0.005,a_gas_ind=0.005,a_oil_trans=0.005,a_oil_res=0.005,a_oil_ind=0.005,a_el_trans=0.005,a_el_res=0.005,a_el_ind=0.005),
                Policy('26',public_ds_capex_share=0.67,share_covered=0.17,prod_subsidy_p_ratio=1.3,gas_ex_cap_end=4631,gas_ex_cap_increase=3,oil_ex_cap_end=76650,oil_ex_cap_increase=30,royalty_rate_gas=0.05,duty_rate_gas=0,royalty_rate_oil=0.05,duty_rate_oil=0,profit_tax_rate=0.15,cons_wedge_end_gas=3.5,cons_wedge_end_oil=1.5,intl_wedge_end_gas=2,intl_wedge_end_oil=1.5,a_gas_trans=0.005,a_gas_res=0.005,a_gas_ind=0.005,a_oil_trans=0.005,a_oil_res=0.005,a_oil_ind=0.005,a_el_trans=0.005,a_el_res=0.005,a_el_ind=0.005),
                Policy('27',public_ds_capex_share=0,share_covered=0,prod_subsidy_p_ratio=1,gas_ex_cap_end=4631,gas_ex_cap_increase=3,oil_ex_cap_end=76650,oil_ex_cap_increase=30,royalty_rate_gas=0.12,duty_rate_gas=0.08,royalty_rate_oil=0.12,duty_rate_oil=0.08,profit_tax_rate=0.3,cons_wedge_end_gas=3.5,cons_wedge_end_oil=1.5,intl_wedge_end_gas=2,intl_wedge_end_oil=1.5,a_gas_trans=0.005,a_gas_res=0.005,a_gas_ind=0.005,a_oil_trans=0.005,a_oil_res=0.005,a_oil_ind=0.005,a_el_trans=0.005,a_el_res=0.005,a_el_ind=0.005),
                Policy('28',public_ds_capex_share=0,share_covered=0,prod_subsidy_p_ratio=1,gas_ex_cap_end=4631,gas_ex_cap_increase=3,oil_ex_cap_end=76650,oil_ex_cap_increase=30,royalty_rate_gas=0.05,duty_rate_gas=0,royalty_rate_oil=0.05,duty_rate_oil=0,profit_tax_rate=0.15,cons_wedge_end_gas=3.5,cons_wedge_end_oil=1.5,intl_wedge_end_gas=2,intl_wedge_end_oil=1.5,a_gas_trans=0.005,a_gas_res=0.005,a_gas_ind=0.005,a_oil_trans=0.005,a_oil_res=0.005,a_oil_ind=0.005,a_el_trans=0.005,a_el_res=0.005,a_el_ind=0.005),
                Policy('29',public_ds_capex_share=0.67,share_covered=0.17,prod_subsidy_p_ratio=1.3,gas_ex_cap_end=4631,gas_ex_cap_increase=3,oil_ex_cap_end=76650,oil_ex_cap_increase=30,royalty_rate_gas=0.12,duty_rate_gas=0.08,royalty_rate_oil=0.12,duty_rate_oil=0.08,profit_tax_rate=0.3,cons_wedge_end_gas=.8,cons_wedge_end_oil=.8,intl_wedge_end_gas=1,intl_wedge_end_oil=1,a_gas_trans=0.005,a_gas_res=0.005,a_gas_ind=0.005,a_oil_trans=0.005,a_oil_res=0.005,a_oil_ind=0.005,a_el_trans=0.005,a_el_res=0.005,a_el_ind=0.005),
                Policy('30',public_ds_capex_share=0.67,share_covered=0.17,prod_subsidy_p_ratio=1.3,gas_ex_cap_end=4631,gas_ex_cap_increase=3,oil_ex_cap_end=76650,oil_ex_cap_increase=30,royalty_rate_gas=0.05,duty_rate_gas=0,royalty_rate_oil=0.05,duty_rate_oil=0,profit_tax_rate=0.15,cons_wedge_end_gas=.8,cons_wedge_end_oil=.8,intl_wedge_end_gas=1,intl_wedge_end_oil=1,a_gas_trans=0.005,a_gas_res=0.005,a_gas_ind=0.005,a_oil_trans=0.005,a_oil_res=0.005,a_oil_ind=0.005,a_el_trans=0.005,a_el_res=0.005,a_el_ind=0.005),
                Policy('31',public_ds_capex_share=0,share_covered=0,prod_subsidy_p_ratio=1,gas_ex_cap_end=4631,gas_ex_cap_increase=3,oil_ex_cap_end=76650,oil_ex_cap_increase=30,royalty_rate_gas=0.12,duty_rate_gas=0.08,royalty_rate_oil=0.12,duty_rate_oil=0.08,profit_tax_rate=0.3,cons_wedge_end_gas=.8,cons_wedge_end_oil=.8,intl_wedge_end_gas=1,intl_wedge_end_oil=1,a_gas_trans=0.005,a_gas_res=0.005,a_gas_ind=0.005,a_oil_trans=0.005,a_oil_res=0.005,a_oil_ind=0.005,a_el_trans=0.005,a_el_res=0.005,a_el_ind=0.005),
                Policy('32',public_ds_capex_share=0,share_covered=0,prod_subsidy_p_ratio=1,gas_ex_cap_end=4631,gas_ex_cap_increase=3,oil_ex_cap_end=76650,oil_ex_cap_increase=30,royalty_rate_gas=0.05,duty_rate_gas=0,royalty_rate_oil=0.05,duty_rate_oil=0,profit_tax_rate=0.15,cons_wedge_end_gas=.8,cons_wedge_end_oil=.8,intl_wedge_end_gas=1,intl_wedge_end_oil=1,a_gas_trans=0.005,a_gas_res=0.005,a_gas_ind=0.005,a_oil_trans=0.005,a_oil_res=0.005,a_oil_ind=0.005,a_el_trans=0.005,a_el_res=0.005,a_el_ind=0.005)]

    print("OUTPUTS AND UNCERTAINTIES LOADED")
    with MultiprocessingEvaluator(model) as evaluator:
        #  warnings.filterwarnings('ignore', 'ImportWarning', )
         results = perform_experiments(model, num_experiments, policies=policies, reporting_interval=1,
                                       evaluator=evaluator)
    ...
    # policies = [Policy('no policy', royalty_rate_gas =.3),
    #         Policy('no gas royalties', royalty_rate_gas =0.0)] 
