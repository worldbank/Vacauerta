...
def VacaMuerta(yr0 = 2020, 
                ...
                duty_rate_gas = .08, 
                duty_rate_oil = .08, 
                price_threshold_gas = 5.0, 
                price_threshold_oil = 60.0, 
                price_floor_gas = 3.75, 
                price_floor_oil = 45.0, 
                T_bond_rate = .0245, 
                arg_sov_risk = .0315, 
    ...
    #format op_ex
    opex_cost ={'product':['Gas','Oil'],'opex':[gas_opex/.000146 , oil_opex/.000146]}
    opex_cost = pd.DataFrame(opex_cost)

    p_thr ={'prod_type':['Gas','Oil'],'threshold':[price_threshold_gas*39652 , price_threshold_oil*7330]}
    p_thr = pd.DataFrame(p_thr)

    p_floor ={'prod_type':['Gas','Oil'],'threshold':[price_floor_gas*39652 , price_floor_oil*7330]}
    ...
    return npv_gdp, npv_unsub_gdp, total_ft_gdp, wells_total, gdp_npv_conv_gas,gdp_npv_conv_oil,gdp_npv_unconv_gas,gdp_npv_unconv_oil, ft_npv_conv_gas, ft_npv_conv_oil, ft_npv_unconv_gas, ft_npv_unconv_oil, fiscal_td, exports
...
if __name__ == "__main__":
    ...
    model.uncertainties = [CategoricalParameter('rcp',['CURR','1.5']), #emissions pathway
                            ...
                            RealParameter("us_capex_gas",3000000, 11000000), #Cost of upstream capex expenditure 
                            RealParameter("us_capex_oil",3000000, 11000000), #Cost of upstream capex expenditure 
                            RealParameter("m",2000, 25000), #cost of downstream gas capital expenditure
                            RealParameter("oil_opex",3, 20), #operations costs for gas invetments 
                            RealParameter("gas_opex",3, 20), #operations costs for gas invetments 
    ...
    with MultiprocessingEvaluator(model) as evaluator:
        #  warnings.filterwarnings('ignore', 'ImportWarning', )
         results = perform_experiments(model, num_experiments, reporting_interval=1,
                                       evaluator=evaluator)
